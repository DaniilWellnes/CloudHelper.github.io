<!DOCTYPE html>
<html lang="ru" class="bg-[#09090d] text-white overflow-x-hidden">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CloudAuthKey - Free-trial</title>
    <link rel="stylesheet" href="output.css">
    <link rel="shortcut icon" type="image/jpg" href="favicon.png">
</head>
<body>
    <header>
        <nav class="border-gray-200 px-6 lg:px-6 py-2.5 mb-15">
    <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                <a href="/" class="flex items-center">
                    <img src="logo-1-dark.png" class="mr-3 h-12 mt-2" alt="CloudAuthKey Logo">
                </a>
    </div>
</nav>
</header>

    <section>
<div class="relative flex flex-wrap items-center justify-center min-h-screen md:-m-8 ml-8 md:ml-24">
    <div class="w-full md:w-1/2 md:p-8">
        <div class="md:max-w-lg md:mx-auto">
            <h2 class="mb-7 md:mb-12 text-3xl md:text-6xl font-bold leading-tight text-center">
                Данные от аккаунта с планом "Разработчик"
                <br><br>
                Логин и пароль: demodeveloper
            </h2>
        </div>
    </div>
</div>

    </section>
</body>
</html>